...
const orgAndOrgIpDataProviderRelationTableController = express.Router();
...
orgAndOrgIpDataProviderRelationTableController
  ...
  .delete(async (req, res) => {
    ...
    try {
      const orgAndOrgIpDataProviderRelationExistsOrNotExists =
        await orgAndOrgIpDataProviderRelationTableService.verifyId(Number(Driver.id));
...
export { orgAndOrgIpDataProviderRelationTableController };
