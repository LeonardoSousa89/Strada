        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (organization.length !== 0) {
                for (let cipherDataPosition in organization) {
                    organization[cipherDataPosition].fantasy_name = this.cryptography.decrypt(organization[cipherDataPosition].fantasy_name);
                    organization[cipherDataPosition].corporate_name = this.cryptography.decrypt(organization[cipherDataPosition].corporate_name);
                    ...
                    organization[cipherDataPosition].cnae_main_code = this.cryptography.decrypt(organization[cipherDataPosition].cnae_main_code);
            ...
            if (orgAndAddress.length !== 0) {
                for (let cipherDataPosition in orgAndAddress) {
                    ...
                    orgAndAddress[cipherDataPosition].street_type = this.cryptography.decrypt(orgAndAddress[cipherDataPosition].street_type);
                    orgAndAddress[cipherDataPosition].public_place = this.cryptography.decrypt(orgAndAddress[cipherDataPosition].public_place);
                    orgAndAddress[cipherDataPosition].org_number = this.cryptography.decrypt(orgAndAddress[cipherDataPosition].org_number);
                    orgAndAddress[cipherDataPosition].complement = this.cryptography.decrypt(orgAndAddress[cipherDataPosition].complement);
                    orgAndAddress[cipherDataPosition].neighborhood = this.cryptography.decrypt(orgAndAddress[cipherDataPosition].neighborhood);
            ...
            if (orgAndDriverAndAddress.length !== 0) {
                for (let cipherDataPosition in orgAndDriverAndAddress) {
                    orgAndDriverAndAddress[cipherDataPosition].zip_code = this.cryptography.decrypt(orgAndDriverAndAddress[cipherDataPosition].zip_code);
                    orgAndDriverAndAddress[cipherDataPosition].state = this.cryptography.decrypt(orgAndDriverAndAddress[cipherDataPosition].state);
                    orgAndDriverAndAddress[cipherDataPosition].city = this.cryptography.decrypt(orgAndDriverAndAddress[cipherDataPosition].city);
                }
            }
            ...
            if (orgAndDriverAndContact.length !== 0) {
                for (let cipherDataPosition in orgAndDriverAndContact) {
                    orgAndDriverAndContact[cipherDataPosition].telephone = this.cryptography.decrypt(orgAndDriverAndContact[cipherDataPosition].telephone);
                }
            }
            ...
            if (orgAndDriverAndDocument.length !== 0) {
                for (let cipherDataPosition in orgAndDriverAndDocument) {
                    orgAndDriverAndDocument[cipherDataPosition].cnh = this.cryptography.decrypt(orgAndDriverAndDocument[cipherDataPosition].cnh);
                }
            }
            ...
            if (orgAndDriverAndInformation.length !== 0) {
                for (let cipherDataPosition in orgAndDriverAndInformation) {
                    orgAndDriverAndInformation[cipherDataPosition].starting_km = this.cryptography.decrypt(orgAndDriverAndInformation[cipherDataPosition].starting_km);
                    orgAndDriverAndInformation[cipherDataPosition].final_km = this.cryptography.decrypt(orgAndDriverAndInformation[cipherDataPosition].final_km);
                    orgAndDriverAndInformation[cipherDataPosition].plate = this.cryptography.decrypt(orgAndDriverAndInformation[cipherDataPosition].plate);
                    orgAndDriverAndInformation[cipherDataPosition].notes = this.cryptography.decrypt(orgAndDriverAndInformation[cipherDataPosition].notes);
                }
            }
            return {
                data: {
