    ...
    getAll(page, size) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let cipherDataPosition in data.data) {
                data.data[cipherDataPosition].zip_code = this.cryptography.decrypt(data.data[cipherDataPosition].zip_code);
                data.data[cipherDataPosition].street_type = this.cryptography.decrypt(data.data[cipherDataPosition].street_type);
                data.data[cipherDataPosition].public_place = this.cryptography.decrypt(data.data[cipherDataPosition].public_place);
                data.data[cipherDataPosition].org_number = this.cryptography.decrypt(data.data[cipherDataPosition].org_number);
                data.data[cipherDataPosition].complement = this.cryptography.decrypt(data.data[cipherDataPosition].complement);
                data.data[cipherDataPosition].neighborhood = this.cryptography.decrypt(data.data[cipherDataPosition].neighborhood);
                data.data[cipherDataPosition].county = this.cryptography.decrypt(data.data[cipherDataPosition].county);
