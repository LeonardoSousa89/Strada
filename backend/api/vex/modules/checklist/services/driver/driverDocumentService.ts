...
import { driverDocumentProjection } from "../../entities/driver/projections/driverProjection";
  ...
  implements DbOperations
{
  ...
  async getAll(page?: any, size?: any) {
    ...
    for (let cipherDataPosition in data.data) {
      data.data[cipherDataPosition].cnh = this.cryptography.decrypt(
        data.data[cipherDataPosition].cnh
      );
    }
